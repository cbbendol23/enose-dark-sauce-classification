        ...
        try:
            ...
            with open(filename, "w", newline="") as f:
                ...
                def update_sensor_display():
                    sensor_text = f"MQ2: {self.latest_values[0]}  MQ3: {self.latest_values[1]}  MQ135: {self.latest_values[2]}  MQ136: {self.latest_values[3]}  MQ137: {self.latest_values[4]}  MQ138: {self.latest_values[5]}"
                    self.canvas.itemconfig(self.sensor_display_id, text=sensor_text)
                    if self.gathering:
                        self.after(1000, update_sensor_display)
...
class ExhaustPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.canvas.create_text(400, 200, text="PROCESS: Exhaustion in Progress",
                    font=TEXTFONT, fill="white")
        # Sensor value display placeholder
        self.sensor_display_id = self.canvas.create_text(400, 380, text="MQ2: - MQ3: - MQ135: - MQ136: - MQ137: - MQ138: -", font=("Segoe UI", 18, "bold"), fill="yellow")
    ...
    def start_timer(self, controller):
        self.remaining_time = 600
        # Start sensor value update loop
        self.update_sensor_values()
        self.update_timer(controller)

    def update_sensor_values(self):
        # Try to read latest values from gathered_data.csv
        try:
            df = pd.read_csv("gathered_data.csv")
            if not df.empty:
                vals = df.loc[0, ["MQ2", "MQ3", "MQ135", "MQ136", "MQ137", "MQ138"]].values
                sensor_text = f"MQ2: {vals[0]}  MQ3: {vals[1]}  MQ135: {vals[2]}  MQ136: {vals[3]}  MQ137: {vals[4]}  MQ138: {vals[5]}"
                self.canvas.itemconfig(self.sensor_display_id, text=sensor_text)
        except Exception:
            pass
        # Update every second
